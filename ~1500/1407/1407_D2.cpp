...
// _________«³?«Ç«£«ó«°ªÏª³ª³ª«ªé£¡£¡___________
namespace atcoder {
...
int e2() { return 0; }
int f1(int x, int y) { return min(x, y); }
int f2(int x, int y) { return max(x, y); }
 
main() {
    INT(n);
    VEC(int, a, n);
    vi dp(n, inf<int>);
    atcoder::segtree<int, f1, e1> seg(a);
    atcoder::segtree<int, f2, e2> seg2(a);
    dp[0] = 0;
    queue<int> q({0});
    auto er = [&](int i) { seg.set(i, e1()), seg2.set(i, e2()); };
    er(0);
    while(!q.empty()) {
        int now = q.front();
        q.pop();
        ...
        if(chmin(dp[now + 1], dp[now] + 1)) q.emplace(now + 1), er(now + 1);
        if(a[now + 1] > a[now]) {
            int nxt = now + 1;
            while(1) {
                int t = a[nxt];
                nxt = seg.max_right(nxt, [&](int k) { return k >= t; });
                if(nxt == n or seg.get(nxt) == e1()) break;
                if(chmin(dp[nxt], dp[now] + 1)) er(nxt), q.emplace(nxt);
                if(a[nxt] <= a[now]) break;
            }
        } else if(a[now + 1] < a[now]) {
            if(chmin(dp[now + 1], dp[now] + 1)) q.emplace(now + 1), er(now + 1);
            int nxt = now + 1;
            while(1) {
                int t = a[nxt];
                nxt = seg2.max_right(nxt, [&](int k) { return k <= t; });
                if(nxt == n or seg2.get(nxt) == e2()) break;
                if(chmin(dp[nxt], dp[now] + 1)) er(nxt), q.emplace(nxt);
                if(a[nxt] >= a[now]) break; 
            }
        }
